...
public class PlantWateringService extends IntentService {
    ...
    private void handleActionWaterPlants() {
        ...
        contentValues.put(COLUMN_LAST_WATERED_TIME, timeNow);
    ...
    public void handleActionUpdatePlantWidgets() {
        ...
        int imgSrc = R.drawable.grass;
        if(cursor!= null && cursor.getCount() > 0) {
            long wateredTime = cursor.getLong(cursor.getColumnIndex(COLUMN_LAST_WATERED_TIME));
            long creationTime = cursor.getLong(cursor.getColumnIndex(COLUMN_CREATION_TIME));
            int type = cursor.getInt(cursor.getColumnIndex(COLUMN_PLANT_TYPE));
            long currentTime = System.currentTimeMillis();

            cursor.close();

            imgSrc = PlantUtils.getPlantImageRes(this, currentTime - creationTime, currentTime - wateredTime, type);
